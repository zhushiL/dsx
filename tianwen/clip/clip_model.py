...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, input_resolution, patch_size, width, layers, heads, output_dim):
        ...
        self.input_resolution = input_resolution
        self.output_dim = output_dim
        # 使用卷积来将整幅图分成小 patch ，直接按照 patch 大小将其从里面抓出来
    ...
    def forward(self, x):
        ...
        x = torch.cat([self.class_embedding.to(x.dtype) +     # (batch_size, grid ** 2 + 1, width)
                       torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device), x], dim=1)
        x = x + self.positional_embedding.to(x.dtype)
        x = self.ln_pre(x)

        x = x.permute(1, 0 ,2)    # (lenth, batch_size, dim)
        x = self.transformer(x)
        x = x.permute(1, 0, 2)    # (batch_size, lenth, dim)

        x = self.ln_post(x[:, 0, :])   # 第零位置的token作为类别输出 ！！！  (batch_size, dim)
        
        if self.proj is not None: # (batch_size, dim) @ (dim, output_dim)
            x = x @ self.proj     # (batch_size, output_dim)
...
class CLIP(nn.Module):
    ...
    def encode_text(self, text):
        ...
        Return:
            (batch_size, embed_dim) 
        """
        x = self.token_embedding(text).type(self.dtype)    # (batch_size, length, transformer_width)

        x = x + self.positional_embedding.type(self.dtype)
        x = x.permute(1, 0, 2)                             # (length, batch_size, transformer_width)
        x = self.transformer(x)
        x = x.permute(1, 0, 2)                             # (batch_size, length, transformer_width)
        x = self.ln_final(x).type(self.dtype)

        # 从 eot(end-of-token) 中拿特征 eot 是每个序列中的最高数字
        # vocab 里 <eot> pad，但是其在 vocab 里对应的序列靠后，直接取 <eot> 对应的特征
        # (batch_size, embed_dim)
        x = x[torch.arange(x.shape[0]), text.argmax(dim=-1)] @ self.text_projection   # argmax() 取对应维度最大值的索引
...
def build_model(state_dict: dict):
    ...
    convert_weights(model)   # 转换模型精度为半精度
    model.load_state_dict(state_dict)   # 保存模型参数
    return model.eval()

#----------------------------------------------------------------------------------------------------------------------------------------

def clip_loss(logits):
    """ logits: clip cosine similarity 输出矩阵 """
    loss = nn.CrossEntropyLoss()
    loss_image = loss(logits, torch.arange(len(logits)))
    loss_text = loss(logits.T, torch.arange(len(logits.T)))
